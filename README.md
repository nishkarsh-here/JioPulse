...
<p>
  ...
  <img alt="OpenAI" src="https://img.shields.io/badge/AI-OpenAI-412991?logo=openai&logoColor=white">
...
1. **Retrieval (RAG).** Markdown in `app/src/ai/knowledge/` is split into chunks and embedded with `text-embedding-3-small`. At query time the most relevant chunks are pulled back by cosine similarity. The index is built once and cached.
...
The assistant degrades gracefully: with no `OPENAI_API_KEY` set, the app runs exactly as before and the assistant simply asks you to add a key.
...
| AI assistant | OpenAI (chat + embeddings), NumPy retrieval |
| QR codes | segno |
...
- An **OpenAI API key** (only needed for the assistant)
...
# then edit .streamlit/secrets.toml with your Supabase + OpenAI keys
...
- **App → Streamlit Community Cloud.** Point it at `app/app.py` and add `SUPABASE_URL`, `SUPABASE_KEY` and `OPENAI_API_KEY` under *Secrets*. After deploying, update the public app URL in `landing/templates/index.html` and `app/src/components/dialog_share_subject.py`.
...
| `OPENAI_API_KEY` | secrets **or** environment | Powers the AI assistant |
